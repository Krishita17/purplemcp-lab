...
![Attack modules](https://img.shields.io/badge/attack%20modules-28-ffadc6?style=flat-square)
![Guardrails](https://img.shields.io/badge/guardrails-23-a9d6f5?style=flat-square)
...
break them with 28 real, runnable exploits, then harden them with a reusable guardrails library.**
...
> (weather/crypto, Tavily web search, VirusTotal + AbuseIPDB threat-intel, DNS/IP lookups), five new
> attacks (insecure JWT, XXE, regex injection, open redirect, CRLF header injection), a live **security-metrics** dashboard
...
| 🔴 **Attack** *(lab only)* | Intentionally-vulnerable MCP servers + working exploits for **28** threat classes, so you can *watch* the protocol get abused — never simulated, always runnable. |
| 🔵 **Defend** | A reusable hardening library (**23 guardrails**), a hardened twin of every vulnerable server, a static **security scanner**, and a **Defense Lab that runs the fix for real** — read it on one side, watch it block the payload on the other. |
...
> a complete, in-depth guide to the protocol, the 28 attack classes, and the 23 guardrails that stop
...
| Red team | **Attack Lab** | Browse all 28 attacks, **run the real exploit** with live output, and copy/run the commands from a built-in **manual terminal** (lab-gated). |
...
| 28 | **CRLF / header injection** | A CR/LF in a header value forges a second, attacker-controlled header |
...
| CRLF / header injection | `guardrails.headers.safe_header_value()` — reject CR/LF + control characters |
